...
def create_target_checkerboard_yaml(tag_size, target_path):
    target = {
        'target_type': 'checkerboard', #gridtype
        'targetCols': 6,               #number of internal chessboard corners
        'targetRows': 7,               #number of internal chessboard corners
        'rowSpacingMeters': tag_size,  #size of one chessboard square [m]
        'colSpacingMeters': tag_size   #size of one chessboard square [m]
    }
    with open(target_path, 'w') as f:
        yaml.safe_dump(target, f, default_flow_style=False)
...
if __name__ == "__main__":
    ...
    create_target_checkerboard_yaml(args.tag_size, target_yaml_path)